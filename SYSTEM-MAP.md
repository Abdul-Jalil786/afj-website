...
- **Report persistence via batch commit** — Railway filesystem resets on redeploy, so agent reports written to disk would be lost. After script execution, `run-agent.ts` reads all output files from disk and batch-commits them to GitHub in a **single commit** via the Git Trees API (`POST /git/blobs` → `POST /git/trees` → `POST /git/commits` → `PATCH /git/refs/heads/main`). This produces exactly 1 commit and 1 Railway deployment per Run Now click (previously each file was a separate `updateFileContent()` call = N commits = N deployments).
  ...
  - Response includes `committedFiles` count; batch commit failure is logged but doesn't crash the response
